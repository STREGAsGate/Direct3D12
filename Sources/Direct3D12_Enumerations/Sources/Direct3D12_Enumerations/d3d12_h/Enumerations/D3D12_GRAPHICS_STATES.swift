...
public struct GraphicsStates: OptionSet {
    ...
    ///// Specifies no state.
    //static let none = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_NONE.rawValue)

    ///	Specifies the state of the vertex buffer bindings on the input assembler stage.
    static let iaVertexBuffers = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_IA_VERTEX_BUFFERS.rawValue)
    ///	Specifies the state of the index buffer binding on the input assembler stage.
    static let iaIndexBuffers = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_IA_INDEX_BUFFER.rawValue)
    ///	Specifies the state of the primitive topology value set on the input assembler stage.
    static let iaPrimitiveTopology = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_IA_PRIMITIVE_TOPOLOGY.rawValue)
    ///	Specifies the state of the currently bound descriptor heaps.
    static let descriptorHeap = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_DESCRIPTOR_HEAP.rawValue)
    ///	Specifies the state of the currently set graphics root signature.
    static let graphicsRootSignature = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_GRAPHICS_ROOT_SIGNATURE.rawValue)
    ///	Specifies the state of the currently set compute root signature.
    static let computeRootSignature = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_COMPUTE_ROOT_SIGNATURE.rawValue)
    ///	Specifies the state of the viewports bound to the rasterizer stage.
    static let rsViewports = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_RS_VIEWPORTS.rawValue)
    ///	Specifies the state of the scissor rectangles bound to the rasterizer stage.
    static let rsScissorRects = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_RS_SCISSOR_RECTS.rawValue)
    ///	Specifies the predicate state.
    static let predication = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_PREDICATION.rawValue)
    ///	Specifies the state of the render targets bound to the output merger stage.
    static let omRenderTargets = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_OM_RENDER_TARGETS.rawValue)
    ///	Specifies the state of the reference value for depth stencil tests set on the output merger stage.
    static let omStencilReference = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_OM_STENCIL_REF.rawValue)
    ///	Specifies the state of the blend factor set on the output merger stage.
    static let omBelndFactor = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_OM_BLEND_FACTOR.rawValue)
    ///	Specifies the state of the pipeline state object.
    static let pipelineState = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_PIPELINE_STATE.rawValue)
    ///	Specifies the state of the buffer views bound to the stream output stage.
    static let soTargets = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_SO_TARGETS.rawValue)
    ///	Specifies the state of the depth bounds set on the output merger stage.
    static let omDepthBounds = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_OM_DEPTH_BOUNDS.rawValue)
    ///	Specifies the state of the sample positions.
    static let samplePositions = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_SAMPLE_POSITIONS.rawValue)
    ///	Specifies the state of the view instances mask.
    static let viewInstanceMask = GraphicsStates(rawValue: WinSDK.D3D12_GRAPHICS_STATE_VIEW_INSTANCE_MASK.rawValue)
